...
@Value("${jwt.password.encoder.secret}")
    ...
    public String encode(CharSequence rawPassword) {
        ...
        try {
            byte[] result = SecretKeyFactory.getInstance(SECRET_KEY_INSTANCE).generateSecret(new PBEKeySpec(rawPassword.toString().toCharArray(), secret.getBytes(), iteration, keyLength)).getEncoded();
